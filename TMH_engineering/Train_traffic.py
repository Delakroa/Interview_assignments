...
# Время в пути и время стоянки должны высчитываться автоматически и не должны вводиться пользователем (при этом пользователю они должны быть видны)
...
# - сохранение маршрута;
# Открытие и редактирование раннее созданных маршрутов (также и после перезапуска программы), следует учесть, что маршрут может состоять более чем из двух пунктов.
...
class Train_schedule:
    ...
    def __init__(self, distance, speed, point, departure_time) -> None:
...
def arrival_time(a_time, d_time):
